...
- un `partner` représente un seul établissement de type `restaurant` ou `market`
...
#### `partners`
Rôle : compte professionnel et établissement unique.
...
Aucun autre bucket métier n’est ajouté dans ce blueprint sans validation explicite du MASTER SPEC.
...
6. upload des documents obligatoires dans storage
...
4. émission `domain_event = order.delivered`
5. déverrouillage avis + satisfaction + fidélité
...
2. backend crée `partner_notification_pack_purchases(status=pending)`
3. backend crée `Stripe PaymentIntent`
4. Stripe webhook confirme paiement
5. `payment_status = succeeded`
6. `purchased_at = now()`
7. backend crédite `partner_notification_credit_ledger` du nombre de campagnes incluses
8. émission `domain_event = notification_pack.purchased`
...
6. débit `partner_notification_credit_ledger` avec `delta_credits = -1`
7. envoi push aux destinataires ciblés par la logique de campagne validée en implémentation
8. création `notification_dispatches`
9. `partner_notification_campaigns.status = sent`
10. émission `domain_event = campaign.sent`
...
Séquence :
1. backend calcule le montant autoritaire
...
## 21. Résumé exécutif CTO

Le blueprint officiel Foodiz repose sur :
